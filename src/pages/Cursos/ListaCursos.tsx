...
const ListaCursos: React.FC = () => {
  const { courses, loading, error } = useAppSelector((state: RootState) => state.courses);
  ...
  return (
    <Container maxWidth="lg" sx={{ px: { xs: 1, sm: 2, md: 3 } }}>
      <Paper elevation={3} sx={{ p: { xs: 2, sm: 3 }, mt: 3 }}>
        ...
        <Box sx={{ height: { xs: 350, sm: 400 }, width: '100%', overflowX: 'auto' }}>
          <DataGrid
            rows={courses}
            columns={columns}
            initialState={{
      ...
      >
        <DialogTitle sx={{ fontSize: { xs: '1.1rem', sm: '1.25rem' } }}>Confirmar Exclusão</DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          <Button onClick={() => setDeleteConfirmOpen(false)} size={isMobile ? "small" : "medium"}>Cancelar</Button>
          <Button onClick={confirmDelete} color="error" autoFocus size={isMobile ? "small" : "medium"}>
            Excluir
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog
        ...
        sx={{ '& .MuiDialog-paper': { width: { xs: '100%', sm: '80%' } } }}
      >
        <DialogTitle sx={{ fontSize: { xs: '1.1rem', sm: '1.25rem' } }}>Matricular Aluno</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ mb: 2, fontSize: { xs: '0.9rem', sm: '1rem' } }}>
        ...
        <DialogActions>
          <Button onClick={() => setEnrollmentOpen(false)} size={isMobile ? "small" : "medium"}>Cancelar</Button>
          <Button onClick={confirmEnrollment} color="primary" size={isMobile ? "small" : "medium"}>
            Matricular
          </Button>
        </DialogActions>
      </Dialog>
